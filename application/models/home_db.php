...
class Home_db extends CI_Model{
	...
	public function resizeImage($filename,$width, $height){
		...
		$config['source_image'] = $_SERVER['DOCUMENT_ROOT'].'/rackpack_manage/'.$filename;
		$config['create_thumb'] = FALSE;
		$config['maintain_ratio'] = FALSE;
		$config['width'] = $width;
		$config['height'] = $height;
		$config['new_image'] = $_SERVER['DOCUMENT_ROOT'].'/rackpack_manage/'.$filename;
	...
	public function resizeImagef($filename,$width, $height,$water = ""){
		...
		$config['source_image'] = $_SERVER['DOCUMENT_ROOT'].'/Rackpack/'.$filename;
		$config['create_thumb'] = FALSE;
		$config['maintain_ratio'] = FALSE;
		$config['width'] = $width;
		$config['height'] = $height;
		$config['new_image'] = $_SERVER['DOCUMENT_ROOT'].'/Rackpack/'.$filename;
		$this->image_lib->initialize($config);
		if(!$this->image_lib->resize())
		{
			return $this->image_lib->display_errors();
	...
	function banner_table($db=array())
{
    ...
    foreach($result->result() as $row) {
        ...
        $image="<a href='".$row->banner_link."' target='_blank'><img src='".str_replace('/rackpack_manage','',base_url())."".$row->banner_image."' width='150' width='100'></a>";
    ...
    function gifts_table($db=array())
    {
        ...
        foreach($result->result() as $row) {
            ...
            $image="<img src='".str_replace('/rackpack_manage','',base_url())."".$row->image_path."'>";
    ...
    function client_table($db=array())
    {
        ...
        foreach($result->result() as $row) {
            ...
            $image="<img src='".str_replace('/rackpack_manage','',base_url())."".$row->client_image."' width='150' width='100'>";
	...
	function ourproducts_table($db=array())
	{
		...
		foreach($result->result() as $row) {
			...
			$image="<img src='".str_replace('/rackpack_manage','',base_url())."".$row->image_path."' width='150' width='100'>";
